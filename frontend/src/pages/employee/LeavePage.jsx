...
function ApplyLeaveModal({ onClose }) {
  ...
  const apply = useMutation({
    ...
    onSuccess: () => { toast.success('Leave request submitted'); qc.invalidateQueries(['my-leaves']); qc.invalidateQueries(['dashboard-stats']); onClose(); },
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:50, display:'flex', alignItems:'center', justifyContent:'center', padding:'16px', background:'rgba(4,7,18,0.8)', backdropFilter:'blur(8px)' }}>
      <div style={{ background:'rgba(13,17,30,0.98)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'20px', boxShadow:'0 24px 64px rgba(0,0,0,0.6)', width:'100%', maxWidth:'460px', maxHeight:'92vh', overflowY:'auto', overflow:'hidden' }}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); apply.mutate(form); }} style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'18px' }}>
          <div>
            ...
            <select value={form.type} onChange={f('type')} style={S.input}
              onFocus={e => { e.target.style.borderColor='rgba(244,114,182,0.6)'; e.target.style.boxShadow='0 0 0 3px rgba(244,114,182,0.1)'; }}
              onBlur={e => { e.target.style.borderColor='rgba(255,255,255,0.1)'; e.target.style.boxShadow='none'; }}>
              {LEAVE_TYPES.map(({ value, label }) => <option key={value} value={value} style={{ background:'#0D1117' }}>{label}</option>)}
            </select>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns: isPermission ? '1fr' : '1fr 1fr', gap:'12px' }}>
            <div>
              ...
              <input type="date" required value={form.start_date} onChange={f('start_date')} style={S.input}
                onFocus={e => { e.target.style.borderColor='rgba(244,114,182,0.6)'; e.target.style.boxShadow='0 0 0 3px rgba(244,114,182,0.1)'; }}
                onBlur={e => { e.target.style.borderColor='rgba(255,255,255,0.1)'; e.target.style.boxShadow='none'; }} />
            </div>
            {!isPermission && (
              <div>
                ...
                <input type="date" required value={form.end_date} onChange={f('end_date')} style={S.input}
                  onFocus={e => { e.target.style.borderColor='rgba(244,114,182,0.6)'; e.target.style.boxShadow='0 0 0 3px rgba(244,114,182,0.1)'; }}
                  onBlur={e => { e.target.style.borderColor='rgba(255,255,255,0.1)'; e.target.style.boxShadow='none'; }} />
              </div>
            )}
          ...
          {isPermission && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
              <div>
                ...
                <input type="time" required value={form.start_time} onChange={f('start_time')} style={S.input}
                  onFocus={e => { e.target.style.borderColor='rgba(244,114,182,0.6)'; e.target.style.boxShadow='0 0 0 3px rgba(244,114,182,0.1)'; }}
                  onBlur={e => { e.target.style.borderColor='rgba(255,255,255,0.1)'; e.target.style.boxShadow='none'; }} />
              </div>
              <div>
                <label style={S.label}>To Time</label>
                <input type="time" required value={form.end_time} onChange={f('end_time')} style={S.input}
                  onFocus={e => { e.target.style.borderColor='rgba(244,114,182,0.6)'; e.target.style.boxShadow='0 0 0 3px rgba(244,114,182,0.1)'; }}
                  onBlur={e => { e.target.style.borderColor='rgba(255,255,255,0.1)'; e.target.style.boxShadow='none'; }} />
              </div>
            </div>
          ...
          <div style={{ display:'flex', gap:'12px', paddingTop:'4px' }}>
            ...
            <button type="submit" disabled={apply.isPending} style={{
              ...
              color:'white', background: apply.isPending ? 'rgba(244,114,182,0.5)' : 'linear-gradient(135deg,#F472B6,#EC4899)',
      ...
      <div style={glass}>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr>
                {['Type','Period','Duration','Reason','Status','Reviewer Note',''].map((h) => (
            ...
            <tbody>
              ...
              {!isLoading && !data?.data?.length && <tr><td colSpan={7} style={{ padding:'48px', textAlign:'center', fontSize:'13px', color:'rgba(241,245,249,0.2)' }}>No records found</td></tr>}
              {data?.data?.map((leave) => (
                <tr key={leave.id}
                ...
                >
                  ...
                  <td style={{ ...S.td, maxWidth:'180px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{leave.reason}</td>
